...
def Run(
  ...
  for i in range(0,size):
    ...
    weight_matrix=weight_matrix +  (np.sqrt(np.power(2.0,-e)))*updates[i]
...
# Dataset 
  ...
  for ts in t_val:
    ...
    for exp in g_val:
      ...
      #avg_max_norm=0
      #for ind in range(0,repition):
      #  final_list[ind]=Run(k, d, size, exp,ts)
      ...
      #print(f"{2} Runs Completed for gamma:{exp} and time_skip:{ts} with error_rate {avg_final_list[0,-1]} in {avg_time} seconds")
